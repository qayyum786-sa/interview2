// app/website-maintenance/page.jsx
"use client";
import React from "react";
import Image from "next/image";
import Link from "next/link";
import { Button } from "primereact/button";

export default function MaintenancePage() {
  return (
    <div className="w-full antialiased text-slate-800">
      {/* HERO */}
      <header
        className="relative w-full h-[30rem] flex items-center"
        style={{
          backgroundImage:
            "linear-gradient(135deg, rgba(3,7,18,0.6), rgba(6,95,180,0.25)), url('/maintenance-hero.jpg')",
          backgroundSize: "cover",
          backgroundPosition: "center",
        }}
      >
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-black/20 to-transparent" />
        <div className="relative z-10 max-w-6xl mx-auto px-6">
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold text-white leading-tight drop-shadow-lg">
            Website / App Maintenance
          </h1>
          <p className="mt-4 text-lg sm:text-xl text-white/90 max-w-2xl">
            Keep your product fast, secure and available — proactive maintenance, monitoring and on-demand fixes backed by SLAs.
          </p>

          <div className="mt-6 flex flex-wrap gap-3">
            <a href="#services">
              <Button label="Our Services" className="bg-[#0b66c3] hover:bg-[#095aa3] text-white" />
            </a>
            <a href="#slas">
              <Button label="SLA & Pricing" className="p-button-outlined" />
            </a>
          </div>
        </div>
      </header>

      {/* MAIN */}
      <main className="max-w-6xl mx-auto px-6 -mt-16 space-y-12">
        {/* Overview */}
        <section className="relative bg-white rounded-2xl shadow-xl p-8 sm:p-10 ring-1 ring-slate-100">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div>
              <div className="inline-flex items-center gap-3 mb-4">
                <div className="bg-gradient-to-br from-[#0b66c3] to-[#06b6d4] text-white rounded-full w-12 h-12 flex items-center justify-center shadow-md">
                  <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" aria-hidden>
                    <path d="M12 2v4M12 18v4M4.9 4.9l2.8 2.8M16.3 16.3l2.8 2.8M2 12h4M18 12h4M4.9 19.1l2.8-2.8M16.3 7.7l2.8-2.8" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
                </div>
                <span className="text-slate-500 font-medium uppercase text-xs tracking-wider">Ops</span>
              </div>

              <h2 className="text-2xl md:text-3xl font-bold mb-3">Overview</h2>
              <p className="text-lg text-slate-700 leading-relaxed mb-4">
                Our maintenance service keeps websites and apps secure, performant and up-to-date. We combine proactive monitoring, scheduled maintenance and rapid incident response so your users always have a great experience.
              </p>

              <ul className="grid sm:grid-cols-2 gap-3 mt-4">
                <li className="flex items-start gap-3">
                  <span className="mt-1 inline-flex items-center justify-center rounded-full w-8 h-8 bg-[#eef6ff] text-[#0b66c3] font-semibold">✓</span>
                  <span className="text-sm text-slate-600">Security patches & updates</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="mt-1 inline-flex items-center justify-center rounded-full w-8 h-8 bg-[#f0fdf4] text-[#16a34a] font-semibold">✓</span>
                  <span className="text-sm text-slate-600">Performance tuning & caching</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="mt-1 inline-flex items-center justify-center rounded-full w-8 h-8 bg-[#fff7ed] text-[#f59e0b] font-semibold">✓</span>
                  <span className="text-sm text-slate-600">Backups & disaster recovery</span>
                </li>
                <li className="flex items-start gap-3">
                  <span className="mt-1 inline-flex items-center justify-center rounded-full w-8 h-8 bg-[#fff1f2] text-[#ef4444] font-semibold">✓</span>
                  <span className="text-sm text-slate-600">On-demand fixes & feature support</span>
                </li>
              </ul>
            </div>

            <div className="rounded-xl overflow-hidden shadow-md transform transition-all duration-400 hover:scale-[1.02]">
              <Image
                src="/maintenance-illustration.jpg"
                width={900}
                height={600}
                alt="Maintenance illustration"
                className="w-full h-64 object-cover"
                priority
              />
            </div>
          </div>
        </section>

        {/* Services */}
        <section id="services" className="space-y-6">
          <h3 className="text-2xl font-bold">Services We Provide</h3>
          <p className="text-slate-600">Pick a managed plan or choose a la carte services to match your needs.</p>

          <div className="grid md:grid-cols-3 gap-6">
            {[
              { title: "Proactive Monitoring", text: "24/7 uptime checks, synthetic transactions and alerting." },
              { title: "Security & Patch Management", text: "OS, framework and dependency updates with vulnerability remediation." },
              { title: "Performance Optimization", text: "Caching, bundle analysis and CDN tuning for faster pages." },
              { title: "Backups & Recovery", text: "Automated backups, tested restores and retention policies." },
              { title: "Bug Fixes & Small Enhancements", text: "Priority triage and fixes for production issues." },
              { title: "Monthly Health Reports", text: "Performance, uptime, and incident summaries with recommendations." },
            ].map((s, i) => (
              <div key={i} className="rounded-xl overflow-hidden group bg-white shadow hover:shadow-2xl transition-all duration-300">
                <div className="p-6">
                  <div className="inline-flex items-center justify-center rounded-lg w-12 h-12 mb-4 bg-gradient-to-br from-[#0b66c3] to-[#06b6d4] text-white shadow-md">
                    <span className="font-semibold text-sm">{s.title.split(" ")[0][0]}</span>
                  </div>
                  <h4 className="font-semibold mb-2">{s.title}</h4>
                  <p className="text-sm text-slate-600 mb-4">{s.text}</p>
                  <a className="text-sm text-[#0b66c3] font-medium group-hover:translate-x-1 transition-transform inline-flex items-center gap-2">
                    Learn more
                    <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" aria-hidden>
                      <path d="M5 12h14M12 5l7 7-7 7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                  </a>
                </div>
              </div>
            ))}
          </div>
        </section>

        {/* SLA & Response */}
        <section id="slas" className="space-y-6">
          <h3 className="text-2xl font-bold">SLA & Response Times</h3>
          <p className="text-slate-600">Clear SLAs so you know how fast we act when things go wrong.</p>

          <div className="grid md:grid-cols-3 gap-6">
            {[
              { title: "Priority", text: "Critical outages (site down, checkout broken)", response: "15 - 60 mins", sla: "24/7" },
              { title: "High", text: "Major features broken or large user impact", response: "2 - 4 hours", sla: "Business hours" },
              { title: "Normal", text: "Minor bugs, content updates, routine maintenance", response: "24 hours", sla: "Business hours" },
            ].map((r, i) => (
              <div key={i} className="rounded-xl p-6 border hover:shadow-2xl transition-all duration-300 bg-white">
                <div className="flex items-center justify-between mb-3">
                  <h4 className="font-semibold">{r.title}</h4>
                  <div className="text-xs text-slate-500">{r.sla}</div>
                </div>
                <p className="text-sm text-slate-600 mb-4">{r.text}</p>
                <div className="text-sm font-medium text-[#0b66c3]">Response: {r.response}</div>
              </div>
            ))}
          </div>

          <div className="mt-4 text-sm text-slate-600">
            <strong>Note:</strong> SLAs depend on plan selection and timezone. We provide optional 24/7 support for critical platforms.
          </div>
        </section>

        {/* Monitoring & Tools */}
        <section className="space-y-6">
          <h3 className="text-2xl font-bold">Monitoring & Tools</h3>
          <p className="text-slate-600">We use industry-standard tooling for observability and incident management.</p>

          <div className="grid md:grid-cols-4 gap-6">
            {[
              "Uptime checks & alerts",
              "Application performance monitoring (APM)",
              "Log aggregation & search",
              "Automated backups & restores",
            ].map((t, i) => (
              <div key={i} className="rounded-xl p-5 bg-white shadow hover:shadow-2xl transition-all duration-300">
                <h4 className="font-semibold mb-2">{t}</h4>
                <p className="text-sm text-slate-600">We configure and maintain these tools, and include insights in monthly reports.</p>
              </div>
            ))}
          </div>
        </section>

        {/* Packages */}
        <section id="packages">
          <h3 className="text-2xl font-bold mb-6">Maintenance Packages</h3>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              { name: "Essential", price: "₹8k/mo", items: ["Weekly backups", "Monthly health report", "Basic updates (non-critical)"] },
              { name: "Pro", price: "₹25k/mo", items: ["24/7 monitoring", "Priority fixes", "Performance tuning"] },
              { name: "Enterprise", price: "Custom", items: ["Dedicated support", "SLA & escalation", "Monthly optimization"] },
            ].map((p, i) => (
              <div key={i} className="rounded-2xl p-6 border hover:border-transparent hover:shadow-2xl transition-all duration-300 bg-white">
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-lg font-semibold">{p.name}</h4>
                  <div className="text-sm text-slate-500">{p.price}</div>
                </div>
                <ul className="mb-6 text-sm text-slate-600 space-y-2">
                  {p.items.map((it, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      <span className="text-sm mt-1 text-[#0b66c3]">•</span>
                      <span>{it}</span>
                    </li>
                  ))}
                </ul>
                <div>
                  <Button label="Choose Plan" className="bg-[#0b66c3] text-white w-full hover:bg-[#095aa3]" />
                </div>
              </div>
            ))}
          </div>
        </section>

        {/* CTA */}
        <section className="rounded-2xl p-8 text-center bg-gradient-to-r from-[#f0f9ff] via-white to-[#eef2ff] shadow-lg">
          <h3 className="text-2xl font-bold mb-2">Want worry-free maintenance?</h3>
          <p className="text-slate-600 mb-6 max-w-2xl mx-auto">
            Book a quick call — we’ll audit your current setup and recommend a maintenance plan that fits your risk profile.
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link href="/contact">
              <button className="inline-flex items-center gap-3 px-6 py-3 rounded-full font-medium text-white bg-gradient-to-r from-[#0b66c3] to-[#06b6d4] shadow-lg transform transition hover:-translate-y-1 focus:outline-none">
                Book Audit
                <svg className="w-4 h-4 opacity-90" viewBox="0 0 24 24" fill="none" aria-hidden>
                  <path d="M5 12h14M12 5l7 7-7 7" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </button>
            </Link>

            <a href="#packages" className="inline-block">
              <Button label="See Packages" className="p-button-outlined" />
            </a>
          </div>
        </section>
      </main>
      {/* ===== FOOTER ===== */}
      <footer className="bg-[#0056d2] text-white px-10 py-8 mt-10">
        <div className="w-full flex flex-col md:flex-row md:justify-between md:items-start gap-10 md:gap-20">
          {/* Left Section */}
          <div className="flex flex-col items-start justify-start w-full md:w-1/3">
            <Image
              src="/luminatewhitelogo.png"
              alt="Luminate Web Solutions"
              width={180}
              height={70}
              className="mb-3"
            />
            <p className="text-sm leading-6">
              © 2024-25 <span className="font-semibold">Luminate Web Solutions</span>.
              <br />
              All rights reserved.
            </p>
          </div>

          {/* Quick Links */}
          <div className="flex flex-col w-full md:w-1/3 text-left">
            <h4 className="font-bold text-lg mb-3">Quick Links</h4>
            <ul className="list-none space-y-2 text-base">
              <li className="hover:text-blue-300 transition-all duration-200 cursor-pointer">Home</li>
              <li className="hover:text-blue-300 transition-all duration-200 cursor-pointer">About Us</li>
              <li className="hover:text-blue-300 transition-all duration-200 cursor-pointer">Contact Us</li>
              <li className="hover:text-blue-300 transition-all duration-200 cursor-pointer">Solutions</li>
              <li className="hover:text-blue-300 transition-all duration-200 cursor-pointer">Career</li>
            </ul>
          </div>

          {/* Contact Info */}
          <div className="flex flex-col w-full md:w-1/3 text-left">
            <h4 className="font-bold text-lg mb-3">Contact Info</h4>
            <p className="leading-7">
              Dubai, UAE
              <br />
              Hyderabad, Telangana, India
            </p>
            <div className="mt-3 space-y-2 text-base">
              <p className="flex items-center gap-2">
                <span>📞</span> +971 56 674 9492
              </p>
              <p className="flex items-center gap-2">
                <span>📞</span> +91 720 734 7455
              </p>
              <p className="flex items-center gap-2">
                <span>✉️</span> info@luminatewebsdl.com
              </p>
            </div>
          </div>
        </div>

        <hr className="my-6 border-white/30" />
        <p className="text-center text-sm opacity-80">
          Designed & Developed by <span className="font-semibold">Luminate Web Solutions</span>
        </p>
      </footer>
    </div>
  );
}
